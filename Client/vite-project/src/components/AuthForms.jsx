    ...
    try {
      if (isLogin) {
        ...
        if (res && res.token) {
          ...
          // שמירת שם המשתמש להצגה ב-Header
          if (res.user) {
        ...
        if (res && res.token) {
          ...
          setSuccessMessage('נרשמת בהצלחה! מעביר אותך ליצירת פרופיל... 🎉');
          setIsLoading(false);
          
          // המתנה קצרה והעברה ליצירת פרופיל
          setTimeout(() => {
            navigate('/musician/create');
          }, 1500);
        } else {
          // Registration succeeded but no token returned
          // שמירת שם המשתמש מהטופס
          localStorage.setItem('userName', `${formData.firstName} ${formData.lastName}`);
