...
export default function CreateMusicianProfile() {
  ...
  // navigate hook להפניה לדפים אחרים
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // הכנת הנתונים לשליחה
      // הערה: בשלב זה אנו שולחים רק נתונים טקסטואליים
      // תמיכה בהעלאת קבצים תתווסף בעתיד עם multer
      ...
      const payload = {
        ...
        profilePicture: profilePreview || '', // כרגע URL מקומי, בעתיד יהיה URL מהשרת
        galleryPictures: [], // בעתיד יכיל URLs מהשרת
        galleryVideos: [] // בעתיד יכיל URLs מהשרת
      };
      
      const res = await api.updateMusicianProfile(payload);
      setSuccess(res.message || 'הפרופיל נשמר בהצלחה! 🎉');
      
      // אופציונלי: הפניה לדף הבית או לדף הפרופיל
      // setTimeout(() => navigate('/'), 2000);
    } catch (err) {
      console.error('Error saving musician profile:', err);
      const msg = (err && (err.message || err.error || err.msg)) ? (err.message || err.error || err.msg) : 'שגיאה בשמירת הפרופיל';
      setError(msg);
    } finally {
      setLoading(false);
    }
